...
from .util_api import float_tensor_2_bool_tensor, out_adaptor, check_input_dtype
...
def split(tensor, split_size_or_sections, dim=0):
    ...
    if isinstance(split_size_or_sections, int) and tensor.shape[dim] % split_size_or_sections == 0:
        return mindspore.numpy.split(tensor, tensor.shape[dim] // split_size_or_sections, dim)
    if isinstance(split_size_or_sections, int) and tensor.shape[dim] % split_size_or_sections != 0:
...
def x2ms_sum(input, dim=None, keepdim=False, dtype=None):
...
def stack(tensors, dim=0, out=None):
    ...
    result = mindspore.numpy.stack(tensors, axis=dim)
...
def scatter(input, dim, index, src, reduce):
...
def randint(*args, **kwargs):
    if len(args) >= 2 and isinstance(args[0], int) and isinstance(args[1], int) or \
            (len(args) == 1 and "high" in kwargs):
...
def is_floating_point(input):
    return input.dtype in mindspore.dtype.float_type
