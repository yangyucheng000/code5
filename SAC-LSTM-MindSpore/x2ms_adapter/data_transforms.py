...
class Resize(mindspore.dataset.vision.Resize):
...
class Compose(mindspore.dataset.transforms.Compose):
...
class RandomResizedCrop(mindspore.dataset.vision.RandomResizedCrop):
    interpolation_map = {
...
class RandomResizedCropAndInterpolation(mindspore.dataset.vision.RandomResizedCrop):
    interpolation_map = {
...
class RandomHorizontalFlip(mindspore.dataset.vision.RandomHorizontalFlip):
    def __init__(self, p=0.5):
        super().__init__(prob=p)


class RandomVerticalFlip(mindspore.dataset.vision.RandomVerticalFlip):
    def __init__(self, p=0.5):
        super().__init__(prob=p)
...
class ToTensor(mindspore.dataset.vision.ToTensor):
    def __call__(self, img):
...
class ColorJitter(mindspore.dataset.vision.RandomColorAdjust):
    def __init__(self, brightness=0, contrast=0, saturation=0, hue=0):
        super().__init__(brightness=brightness, contrast=contrast, saturation=saturation, hue=hue)


class RandomRotation(mindspore.dataset.vision.RandomRotation):
...
class RandomErasing(mindspore.dataset.vision.RandomErasing):
    def __init__(self, p=0.5, scale=(0.02, 0.33), ratio=(0.3, 3.3), value=0, inplace=False):
        super().__init__(prob=p, scale=scale, ratio=ratio, value=value, inplace=inplace)


class TenCrop(mindspore.dataset.vision.TenCrop):
    def __init__(self, size, vertical_flip=False):
        super().__init__(size=size, use_vertical_flip=vertical_flip)


class RandomAffine(mindspore.dataset.vision.RandomAffine):
...
class RandomApply(mindspore.dataset.transforms.RandomApply):
    def __init__(self, transforms, p=0.5):
        super().__init__(transforms=transforms, prob=p)


class RandomPosterize(mindspore.dataset.vision.RandomPosterize):
...
class RandomGrayscale(mindspore.dataset.vision.RandomGrayscale):
    def __init__(self, p=0.1):
        super().__init__(prob=p)


class RandomPerspective(mindspore.dataset.vision.RandomPerspective):
...
class Pad(mindspore.dataset.vision.Pad):
...
class LinearTransformation(mindspore.dataset.vision.LinearTransformation):
...
class RandomSolarize(mindspore.dataset.vision.RandomSolarize):
...
class RandomChoice(mindspore.dataset.transforms.RandomChoice):
...
class AutoAugment(mindspore.dataset.vision.AutoAugment):
...
class RandomAdjustSharpness(mindspore.dataset.vision.RandomAdjustSharpness):
    def __init__(self, sharpness_factor, p=0.5):
        super().__init__(degree=sharpness_factor, prob=p)


class RandomAutocontrast(mindspore.dataset.vision.RandomAutoContrast):
    def __init__(self, p=0.5):
        super().__init__(prob=p)


class RandomEqualize(mindspore.dataset.vision.RandomEqualize):
    def __init__(self, p=0.5):
        super().__init__(prob=p)


class RandomInvert(mindspore.dataset.vision.RandomInvert):
    def __init__(self, p=0.5):
        super().__init__(prob=p)
