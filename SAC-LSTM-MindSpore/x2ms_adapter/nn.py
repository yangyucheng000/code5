...
from .loss import legacy_parameter
...
def pad_sequence(sequences, batch_first=False, padding_value=0.0):
    ...
    for seq in sequences:
        new_sequences.append(nn_functional.x2ms_pad(seq, (0, 0, 0, max_seq_len - seq.shape[0]), value=padding_value))
    if not batch_first:
        return mindspore.ops.Stack(axis=1)(new_sequences)
    return mindspore.ops.stack(new_sequences)
...
class Bilinear(mindspore.nn.BiDense):
    def __init__(self, in1_features, in2_features, out_features, bias=True, device=None, dtype=None):
        super().__init__(in1_channels=in1_features, in2_channels=in2_features, out_channels=out_features, has_bias=bias)
