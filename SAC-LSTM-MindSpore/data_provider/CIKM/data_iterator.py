...
rootPath = x2ms_adapter.tensor_api.split(os.path, curPath)[0]
sys.path.append(rootPath)
rootPath = x2ms_adapter.tensor_api.split(os.path, rootPath)[0]
sys.path.append(rootPath)
...
class CIKM_Datasets:
    ...
    def __getitem__(self, index):
        ...
        x2ms_adapter.tensor_api.sort(files)   #对列表进行排序 这里有点没对
        imgs = []
        for file in files:
            imgs.append(imread(self.folds+file)[:,:,np.newaxis])
        imgs = np.stack(imgs,0)
        imgs = x2ms_adapter.from_numpy(imgs)
        in_imgs = imgs[:5]
        out_imgs = imgs[5:]
...
class ToTensor(object):
    ...
    def __call__(self, sample):
        video_x = sample
        video_x = x2ms_adapter.tensor_api.transpose(video_x, (0, 3, 1, 2))
        video_x = np.array(video_x)
        return x2ms_adapter.tensor_api.x2ms_float(x2ms_adapter.from_numpy(video_x))


class CIKM_Datasets_custom:
    ...
    def __getitem__(self,idx):
        item_ifo_list = x2ms_adapter.tensor_api.split(self.file_list[idx], ',')
        # print(item_ifo_list)
        ...
        for i in range(end - begin):
            ...
            print("输入路径图片：", str(item_ifo_list[0]) + file_name)
...
def data_process(filename,data_type,dim=None,start_point = 0):
    ...
    with open(filename) as fr:
        ...
        for i in range(start_point+1,sample_num+1):
            ...
            line = x2ms_adapter.tensor_api.split(fr.readline().strip(), ' ')
            cate = x2ms_adapter.tensor_api.split(line[0], ',')
            id_label = [cate[0]]
            record = [int(cate[2])]
            length = len(line)
