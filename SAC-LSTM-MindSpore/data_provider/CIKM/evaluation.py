...
def seq_hss_csi_test(test_model_list,model_names,is_java=True,is_plot=True):
    ...
    for i,model in enumerate(test_model_list):
        ...
        mean_hss = x2ms_np.mean(hss, 0)
        mean_csi = x2ms_np.mean(csi, 0)
...
def eval_test(true_fold,pred_fold,eval_type):
    ...
    for index in sample_indexes:
        ...
        if eval_type == 'mse':
            ...
            sample_res = x2ms_np.mean(np.square(pred_imgs - true_imgs))
        elif eval_type == 'mae':
            # sample_res = np.abs(pred_imgs - true_imgs).mean()
            sample_res = x2ms_np.mean(np.abs(pred_imgs - true_imgs))
        elif eval_type == 'ssim':
            sample_res = 0
            for t in range(10):
            ...
            sample_res = x2ms_adapter.tensor_api.sqrt(np, x2ms_np.mean(np.square(pred_imgs - true_imgs)))
