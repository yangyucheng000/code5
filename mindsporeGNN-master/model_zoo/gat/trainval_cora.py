...
def main():
    ...
    for e in range(epochs):
        beg = time.time()
        train_net.set_train()
        train_net(node_feat, node_label, train_mask, *g.get_graph())
        end = time.time()
        dur = end - beg
        if e >= warm_up:
            total = total + dur
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='GAT for whole-graph classification')
