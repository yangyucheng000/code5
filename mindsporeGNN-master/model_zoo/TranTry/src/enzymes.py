...
from mindspore_gl.dataset import BaseDataSet
...
class Enzymes(BaseDataSet):
    ...
    def __init__(self, root):
        ...
        # adding edge_feat to the list and wait for update
        self._edge_feat_list = []
        self._new_edge_list = []
    ...
    def append_edge_feat(self, edge_feat):
        self._edge_feat_list.append(edge_feat)
        
    def append_new_edge(self, new_edge):
        self._new_edge_list.append(new_edge)
        
    def update_edge_info(self):
        """
        Update edge info from new edge list and edge feat list.
        When the data in new edge list and edge feat list is ready, you should call
        this function to update edge info.
    ...
    @property
    def edge_feat_list(self):
        return self._edge_feat_list

    def edge_feat(self, idx: int):
        return self._edge_feat_list[idx]
    
    @property
    def new_edge_list(self):
        return self._new_edge_list

    def new_edge(self, idx: int):
        return self._new_edge_list[idx]

        # return single graph which is already transformed
    def get_new_single_graph(self, idx) -> Union[MindHomoGraph, np.ndarray]:
        ds = self
        assert idx < ds.graph_count, "Index out of range"
        res = MindHomoGraph()
        # reindex to 0
        coo_array = ds.new_edge(idx)
        res.set_topo_coo(coo_array)
        res.node_count = ds.graph_nodes[idx + 1] - ds.graph_nodes[idx]
        res.edge_count = res.adj_coo[0].size
        return res
