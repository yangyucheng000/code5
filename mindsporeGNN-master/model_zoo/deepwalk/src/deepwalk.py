...
class BatchRandWalk:
    ...
    def __call__(self, nodes):
        walks = random_walk_unbias_on_homo(self.graph, np.array(nodes, int), self.walk_len)
        src_list, pos_list = [], []
        for walk in walks:
        ...
        negs = np.array(negs, dtype=int)
        src, pos = np.reshape(src, [-1, 1]), np.reshape(pos, [-1, 1])
        dsts = np.concatenate([pos, negs], 1)

        # here we padding with fill_value = n_nodes
        node_mask = np.concatenate([np.ones((pair_count), int), np.zeros((self.padded_size-pair_count), int)])
        src = np.concatenate([src, np.ones((self.padded_size-pair_count, 1), int)*self.fill_value])
        dsts = np.concatenate([dsts,
                               np.ones((self.padded_size-pair_count, 1 + self.neg_num), int)*self.fill_value])
