...
def generate_graph(node_count, edge_prob=0.1):
    ...
    edge_ids = np.array(list(range(edge_count))).astype(int)
    generated_graph.set_topo(CsrAdj(csr_mat.indptr.astype(int), \
                                    csr_mat.indices.astype(int)), node_dict, edge_ids)
...
class TestSamplers(unittest.TestCase):
    ...
    def test_sage_sampling(self):
        ...
        sage_sampler_on_homo(homo_graph=self.graph, seeds=nodes[:10].astype(int), neighbor_nums=[2, 2])

    def test_random_walk(self):
        nodes = np.arange(0, self.node_count)
        random_walk_unbias_on_homo(homo_graph=self.graph, seeds=nodes[:30].astype(int), walk_length=10)
