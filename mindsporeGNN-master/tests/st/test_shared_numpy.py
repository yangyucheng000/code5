...
def test_queue():
    ...
    arr = np.ones([10000, 600], dtype=float)
    queue.put(arr)
    ret = queue.get()
...
def test_create_from_numpy():
    ...
    Expectation: success.
    """
    numpy_array = np.ones([100000, 600], dtype=float)
...
def test_create_from_shape():
    """
        ...
        Description: target_size = [10000, 600] , dtype = int
        Expectation: success.
        """
    target_size = [10000, 600]
    shared_array = shared_numpy.SharedNDArray.from_shape(target_size, dtype=int)
    shared_array[0, 10:30] = 1
    assert all(shared_array[0, 10:30] == 1)
    ms.Tensor.from_numpy(shared_array)
