...
def test_pad_array2d():
    ...
    Expectation: Output result
    """
    pad_op = PadArray2d(dtype=float, mode=PadMode.CONST, direction=PadDirection.COL, size=(10, 2), fill_value=0)
...
def test_csr_graph():
    ...
    node_feat = np.array([[1, 2, 3, 4], [2, 4, 1, 3], [1, 3, 2, 4],
                          [9, 7, 5, 8], [8, 7, 6, 5], [8, 6, 4, 6], [1, 2, 1, 1]], float)
    n_nodes = 7
    n_edges = 8
    src_idx_array = np.array(src_idx, int)
    dst_idx_array = np.array(dst_idx, int)
