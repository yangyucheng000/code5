...
class ModelBase():
    ...
    def describe_network(self, network):
        ...
        msg += 'Params number: {}'.format(sum(map(lambda x: x.numel(), network.get_parameters()))) + '\n'
        msg += 'Net structure:\n{}'.format(str(network)) + '\n'
        return msg
    ...
    def describe_params(self, network):
        ...
        for name, param in network.parameters_dict().items():
            if not 'num_batches_tracked' in name:
                ...
                msg += ' | {:>6.3f} | {:>6.3f} | {:>6.3f} | {:>6.3f} | {} || {:s}'.format(v.mean().numpy(), v.min().numpy(), v.max().numpy(), v.std().numpy(), v.shape, name) + '\n'
        return msg
    ...
    def save_network(self, save_dir, network, network_label, iter_label):
        ...
        network = self.get_bare_model(network)
        ms.save_checkpoint(network, save_filename)
    ...
    def load_network(self, load_path, network, strict=True, param_key='params', is_gmflow=False):
        ...
        if strict:
            if is_gmflow:
                state_dict = ms.load_checkpoint(load_path)['model']
            else:
                state_dict = ms.load_checkpoint(load_path)
            if param_key in state_dict.keys():
                state_dict = state_dict[param_key]
            ms.load_param_into_net(network, state_dict, strict_load=strict)
        else:
            state_dict_old = ms.load_checkpoint(load_path) #['state_dict']
            if param_key in state_dict_old.keys():
                state_dict_old = state_dict_old[param_key]
            state_dict = network.parameters_dict()
    ...
    def update_E(self, decay=0.999):
        ...
        netG_params = dict(netG.parameters_dict())
        netE_params = dict(self.netE.parameters_dict())
        for k in netG_params.keys():
            netE_params[k].data.mul(decay).add(netG_params[k].data)*(1-decay)
