...
class ModelSRSCRSG(ModelPlain):
    ...
    def define_optimizer(self):
        ...
        if self.opt_train['G_optimizer_type'] == 'adam':
            self.G_optimizer = optim.Adam(G_optim_params, learning_rate=self.opt_train['G_optimizer_lr'],
                                       beta1=self.opt_train['G_optimizer_betas'],
                                    weight_decay=self.opt_train['G_optimizer_wd'])
        elif self.opt_train['G_optimizer_type'] == 'adamw':
            self.G_optimizer = optim.AdamWeightDecay(G_optim_params, learning_rate=self.opt_train['G_optimizer_lr'], eps=1e-8, weight_decay=0.01)
    ...
    def pad(self, img, ratio=32):
        if len(img.shape) == 5:
            ...
            for i in range(n1):
                ...
                pw = ((w - 1) // ratio + 1) * ratio
                padding = (0, pw - w, 0, ph - h)
                img1 = O.pad(img1, padding, mode='replicate')
    ...
    def netG_forward(self, is_train=True):
        ...
        if is_train:
            ...
            assert self.all_time_rsc.shape[1]//2 == 9, self.all_time_rsc.shape[1]
            E_list = []
