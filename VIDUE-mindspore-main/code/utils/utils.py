...
def np2Tensor(*args, rgb_range=255, n_colors=1):
    def _np2Tensor(img):
        ...
        tensor=np_transpose*(rgb_range / 255) # ms dataset needs np output instead of tensor
...
def eventnp2Tensor(*args):
    def _np2Tensor(img):
        ...
        np_transpose = np.ascontiguousarray(img.transpose((2, 0, 1)))  # NHWC -> NCHW
        #tensor = torch.from_numpy(np_transpose).float()  # numpy -> tensor
        tensor=np_transpose # ms dataset needs np output instead of tensor
        return tensor
...
def calc_psnr(img1, img2, rgb_range=1., shave=4):
    if isinstance(img1, ms.Tensor):
        img1 = img1[:, :, shave:-shave, shave:-shave]
        img1 = img1.numpy()
    if isinstance(img2, ms.Tensor):
...
def calc_grad_sobel(img, device='cuda'):
    ...
    sobel_filter_X = ms.Tensor.from_numpy(sobel_filter_X).float().to(device)
    sobel_filter_Y = ms.Tensor.from_numpy(sobel_filter_Y).float().to(device)
    ...
    grad = O.sqrt(grad_X.pow(2) + grad_Y.pow(2))
...
def calc_meanFilter(img, kernel_size=11, n_channel=1, device='cuda'):
    ...
    mean_filter_X = ms.Tensor.from_numpy(mean_filter_X).float().to(device)
    new_img = O.zeros_like(img)
    for i in range(n_channel):
        new_img[:, i:i + 1, :, :] = F.conv2d(img[:, i:i + 1, :, :], mean_filter_X, bias=None,
                                             stride=1, padding=kernel_size // 2)
    return new_img

def label_to_levels(label, num_classes, dtype=ms.float32):
    ...
    if not label <= num_classes-1:
        raise ValueError('Class label must be smaller or '
                         ...
                         % (num_classes-1, label))
    if isinstance(label, ms.Tensor):
        int_label = int(label.item())
    else:
        int_label = int(label)

    levels = [1]*int_label + [0]*(num_classes - 1 - int_label)
    levels = ms.tensor(levels, dtype=dtype)
    return levels


def levels_from_labelbatch(labels, num_classes, dtype=ms.float32):
    """
...
def proba_to_label(probas):
    ...
    predicted_labels = O.sum(predict_levels, 1)
